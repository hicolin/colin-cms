...
<div class="x-body">
    ...
    <xblock>
        <button class="layui-btn layui-btn-danger" onclick="batch_del('<?= Url::to(['member/batch-del']) ?>')"><i class="layui-icon"></i>批量删除</button>
        <button class="layui-btn" onclick="x_admin_show('添加用户','<?=  Url::to(['member/create']) ?>')"><i class="layui-icon"></i>添加</button>
        ...
        <?php foreach ($models as $list): ?>
            <tr>
                ...
                <td class="td-manage">
                    ...
                    <a title="删除" onclick="del(this,'<?= $list['id'] ?>', '<?= Url::to(['member/del']) ?>')" href="javascript:;">
                        <i class="layui-icon">&#xe640;</i>
                    </a>
                </td>
            </tr>
