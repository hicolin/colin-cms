...
<div class="x-body">
    <div class="layui-row">
        <form class="layui-form layui-col-md12 x-so">
            ...
            <input type="text" name="search[route_name]" placeholder="路由名"
                   value="<?= isset($search['route_name']) ? $search['route_name'] : '' ?>" autocomplete="off"
                   class="layui-input">
            <input type="text" name="search[route]" placeholder="路由"
                   value="<?= isset($search['route']) ? $search['route'] : '' ?>" autocomplete="off"
                   class="layui-input">
            ...
            <button class="layui-btn" type="button"
                    onclick="location.href= '<?= Url::to([$this->context->id . '/index']) ?>'">
    ...
    <xblock>
        <button class="layui-btn layui-btn-danger"
                onclick="batch_del('<?= Url::to([$this->context->id . '/batch-del']) ?>')"><i class="layui-icon"></i>批量删除
        </button>
        <span class="x-right" style="line-height:40px">共有数据：<span
                    class="count_num"><?= $pagination->totalCount ?></span> 条 ( <?= $pagination->getPageCount() ?>
            页 )</span>
    </xblock>
    <table class="layui-table">
        ...
        <?php foreach ($models as $list): ?>
            <tr>
                <td>
                    <div class="layui-unselect layui-form-checkbox" lay-skin="primary" data-id='<?= $list['id'] ?>'><i
                                class="layui-icon">&#xe605;</i></div>
                </td>
                <td><?= $list['id'] ?></td>
                ...
                <td class="td-manage">
                    ...
                    <a title="删除"
                       onclick="del(this,'<?= $list['id'] ?>', '<?= Url::to([$this->context->id . '/del']) ?>')"
                       href="javascript:;">
                        <i class="layui-icon">&#xe640;</i>
                    </a>
                </td>
